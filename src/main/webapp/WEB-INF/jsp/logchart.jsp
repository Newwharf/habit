		...
		<%if(actionList.size()>0){ %>
			...
			%>
				...
				<div>
					...
					<font><%=actionLog!=null?"最近数据，"+actionLog.getfScoreweight()+"kg/"+actionLog.getiScorenum()+action.getvUnit():"暂无数据" %></font>
				</div>
				</div>
				<%}else{ %>
				<div class="target_in"  data="<%=action.getiId()%>" isJump="<%=actionLog!=null?true:false%>">
				...
				<div>
					<span><%=action.getvName() %></span><br>
					<font><%=actionLog!=null?"最近数据，"+actionLog.getfScoreweight()+"kg/"+actionLog.getiScoretime()+"秒":"暂无数据" %></font>
				</div>
				</div>
				<%} %>
			<%} %>
